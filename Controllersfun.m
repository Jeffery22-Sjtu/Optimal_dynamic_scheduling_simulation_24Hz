%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% [CONFIDENTIAL]
% S-function of the controller of loop 1 and loop 3
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [sys,x0,str,ts,simStateCompliance] = Controllersfun(t,x,u,flag,delta_t,delta_tc,A1,R1,A2,R2,alpha,uini,trigger,L1sp,L2sp,usp)
...
%x(12) uc
sizes.NumOutputs     = 10;
sizes.NumInputs      = 4;   % L1, L2, LR, send1
...
% global current future
x(4)=x(1);
if (rem(x(2),delta_tc/delta_t)==0)
if (x(2)<=3*delta_tc/delta_t)||(x(2)==round(u(4))) % x(2) is always equal to u(4) here   
    L1=u(1);
    L2=u(2);
    LR=u(3);
    %common constant
    ...
    for j=1:max_inter
        Lp(j+1,:)= SSMA*Lp(j,:)'+SSMB*uu;
            ...
            elseif Lp(j+1,1)>=50
                ...
                SSMA(1,1)=1-delta_tc*A*D*1/(sqrt(L1));
                SSMA(2,1)=delta_tc*B*D*1/(sqrt(L1));
                SSMB(1,1)=delta_tc*A*alpha;
            end
            ...
            elseif Lp(j+1,2)>=50
                ...
                SSMA(2,2)=1-delta_tc*B*E*1/(sqrt(L2));
                SSMA(3,2)=delta_tc*C*E*1/(sqrt(L2));
                SSMA(2,1)=delta_tc*B*D*1/(sqrt(L1));                
            end
