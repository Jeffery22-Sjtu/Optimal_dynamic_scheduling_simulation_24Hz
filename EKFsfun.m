function [sys,x0,str,ts,simStateCompliance] = EKFsfun(t,x,u,flag,delta_t,delta_tc,LiniEKF,TraceS,A1,R1,A2,R2,AR,alpha)
...
bX=[jX(1)-A*D*delta_tc*sqrt(jX(1))+delta_tc*A*alpha*U;delta_tc*B*D*sqrt(jX(1))+jX(2)-delta_tc*B*E*sqrt(jX(2));delta_tc*C*E*sqrt(jX(2))-delta_tc*C*alpha*U+jX(3)]; 
AJ=[1-delta_tc*A*D*1/(2*sqrt(jX(1))) 0 0;
   delta_tc*B*D*1/(2*sqrt(jX(1)))   1-delta_tc*B*E*1/(2*sqrt(jX(2))) 0;
   0 delta_tc*C*E/(2*sqrt(jX(2))) 1];

bP=AJ*jP*AJ'+Q;
H=[0 1 0];
Kk=bP*H'*(inv(H*bP*H'+R));
bZ=bX(2);  
jX=bX+Kk*(L_M-bZ)*TraceS(x(4)); 
jP=bP-Kk*H*bP;
end
x(1)=jX(1);
