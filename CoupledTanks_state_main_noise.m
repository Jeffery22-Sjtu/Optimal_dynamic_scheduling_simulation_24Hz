...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% [CONFIDENTIAL]
% Wireless control simulation under variable noise levels
...
round_count=0;       % round number of simulations -1, ICCPS simulation:49;             
Tf =12; 
...
Cost3=[];
Cost4=[];
load('link_quality_78.mat');
link8f_ok = link8f;
link2f_ok = link2f;
link3f_ok = link3f;
link5f_ok = link5f;
load('link_quality_84.mat');
link8f_good = link8f;
link2f_good = link2f;
link3f_good = link3f;
link5f_good = link5f;
load('link_quality_75.mat');
link8f_bad = link8f;
link2f_bad = link2f;
link3f_bad = link3f;
link5f_bad = link5f;
...
global Atrace Atrace_first_half Atrace_second_half
yh
%% load TOSSIM link quality traces (various noise level)
trace_count = Tf*sensing_rate*3+1;
Atrace_first_half=[link5f_ok(yh*trace_count+1:(yh+1)*trace_count+200),...
    link2f_ok(yh*trace_count+1:(yh+1)*trace_count+200),...
    link3f_bad(yh*trace_count+1:(yh+1)*trace_count+200),...
    link8f_bad(yh*trace_count+1:(yh+1)*trace_count+200)];

Atrace_second_half=[link5f_bad(yh*trace_count+1:(yh+1)*trace_count+200),...
    link2f_bad(yh*trace_count+1:(yh+1)*trace_count+200),...
    link3f_good(yh*trace_count+1:(yh+1)*trace_count+200),...
    link8f_good(yh*trace_count+1:(yh+1)*trace_count+200)];

Atrace = Atrace_first_half;
...
plot_network_zoomin;
metric_calculate;
save(strcat('data_noise_time_lyap_net_24Hz_long_L1L2_sub',num2str(yh)));
end

%  save('data_noise_time_lyap_net_24Hz_long_L1L2');
%  save('data_noise_time_lyap_net_pri_24Hz_long_L1L2_3');
% save('data_noise_time_periodic_24Hz_long_L1L2');
